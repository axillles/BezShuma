...
class AIProcessor:
    ...
    def _finalize_post(self, raw: str, topic: str) -> str:
        ...
        # Полностью убираем хештеги из основного текста (заменяем `#слово` на `слово`)
        # и удаляем строки, которые состоят преимущественно из хештегов
        cleaned_lines = []
        for line in lines:
            stripped = line.strip()
            if not stripped:
                ...
                continue
            # Если строка почти целиком из хештегов — пропускаем её
        ...
        # Всегда добавляем в конце единственную строку с ровно 3 хештегами
